...
const Connector = () => {
  ...
  const balanceToStr = (balNbr: number) => {
    ...
    const strBalance = balance.toLocaleString(undefined, {
    ...
    <>
          ...
          <div className="w-full">
            <div className="flex">
              <div className="hidden md:flex md:pr-4">
                {router.pathname.startsWith("/profile") ? (
                  <>
                    <span
                      onClick={() => router.push(`/`)}
                      className="pt-1 font-light hover:text-white/80 text-xs w-full justify-end mx-0 flex"
                    >
                      [Go Home]
                    </span>
                  </>
              ...
              <div
                ...
                className="border block border-white rounded-md hover:opacity-80"
              >
                <div className="flex text-xs font-light p-1 px-2.5 items-center m-auto">
                  {bal ? (
                    <span className="flex ">({bal} SOL)</span>
                  ) : (
                    <>
                      <Spinner className="w-[12px] h-[12px] spinner text-[#FFFFFF80] animate-spin fill-white mr-1" />
                    </>
                  )}
            ...
            {router.pathname.startsWith("/profile") ? (
              <>
                <span
                  onClick={() => router.push(`/`)}
                  className="pt-1 font-light hover:text-white/80 text-xs w-full justify-end mx-0 md:hidden flex"
                >
                  [Go Home]
                </span>
              </>
      ...
      <>
        {isDisconnectModalOpen && (
          <>
            <DisconnectModal
              showModal={isDisconnectModalOpen}
              closeModal={handleDisconnectModal}
              ref={DisconnectModalRef}
              addr={address ? address : "not connected"}
            />
          </>
        )}
      </>
